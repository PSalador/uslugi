    ...
    <div class="form-group{{ $errors->has($name) ? ' has-error' : '' }}">
        ...
        <div class="input-group">
            ...
            <input type="hidden" id="lng-{{$slug}}-{{$lang}}" name="{{$inputname}}[lng]"
                   value="{{$value['lng'] or ''}}">		
			<input type="hidden" id="adressjson-{{$slug}}-{{$lang}}" name="{{$inputname}}[adressjson]"
                   value="{{$value['adressjson'] or ''}}">					   
...
document.documentElement.addEventListener("googleMapsLoad", function(e) {
    ...
    autocomplete{{$slug}}{{$lang}}.addListener('place_changed', function () {
		...
		var adressarr = autocomplete{{$slug}}{{$lang}}.getPlace();
		var adressjson = [
						adressarr.address_components[0].long_name,
						adressarr.address_components[1].long_name, 
						adressarr.address_components[2].long_name, 
						adressarr.address_components[3].long_name, 
						adressarr.address_components[4].long_name, 
						adressarr.address_components[5].long_name, 
						adressarr.address_components[6].long_name,
						];
		adressjson=JSON.stringify(adressjson);
        $('#adressjson-{{$slug}}-{{$lang}}').val(adressjson);
    });
